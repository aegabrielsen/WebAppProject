...
def media_uploads(request, handler):
    ...
    file = open(f"public/image/image{image_id}.{extension}", "wb")
    file.write(content)
    file.close()
    ...
    if user:
        ...
        if extension == 'mp4':
            chat_collection.insert_one({"username": "Guest", "message": f'<video width="240" height="240" controls><source src="public/image/image{image_id}.{extension}" type="video/mp4"></video>', "user_browser_id": user_browser_id})
        else:
